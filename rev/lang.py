"""
Zero-dependency sub-microsecond language and script detection used for routing in rev.

Routing only needs one decision: is this English Latin text (optimal for ModernBERT-large 421M),
or does it contain non-Latin scripts or non-English text (requiring mmBERT-base 322M for 100+ languages)?

Benchmarks on MASSIVE (14 languages) show that English-only checkpoints collapse on non-Latin scripts
(Hindi 0.100, Korean 0.103, Swahili 0.103, Tamil 0.113 at 20 options where random is 0.050), while
reporting dangerously high confidence (ECE 0.855). In contrast, mmBERT-base natively maintains high
accuracy across 100+ languages.

Script detection is exact and completes in <1 microsecond.
The Latin-script language detection is a fast stopword and diacritic analyzer.
"""

import re
from typing import Dict, List, Optional, Union

# Unicode blocks that the English (ModernBERT-large) checkpoint cannot read.
_SCRIPT_RANGES = [
...
# Function words for major Latin-script languages.
_STOP = {
...
def _iter_text(state: Union[str, dict, list, None], _depth: int = 0) -> List[str]:
    """Recursively collect string leaves of state."""
...
def state_text(state: Union[str, dict, list, None], max_chars: int = 4000) -> str:
    """Flatten state into text for detection."""
...
def script_profile(text: str) -> Dict[str, float]:
    """Fraction of alphabetic characters per script."""
...
def latin_profile(text: str) -> Dict[str, object]:
    """Analyzes Latin-script text for language signals and diacritics."""
...
def analyse(state: Union[str, dict, list, None]) -> Dict[str, object]:
    """Full language and script detection result for a state."""
...
def is_english(state: Union[str, dict, list, None]) -> bool:
    """True when the English checkpoint can read this state reliably."""
